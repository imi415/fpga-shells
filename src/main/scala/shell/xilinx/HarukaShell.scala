...
package sifive.fpgashells.shell.xilinx.harukashell
...
abstract class HarukaShell(implicit val p: Parameters) extends RawModule {
  ...
  // UART0
  val uart0_rxd_out = IO(Analog(1.W))
  val uart0_txd_in  = IO(Analog(1.W))
  ...
  val jtag_trst    = IO(Analog(1.W)) // TRST_n
  ...
  def connectUART(uart: UARTPortIO): Unit = {
    IOBUF(uart0_rxd_out, uart.txd)
    uart.rxd := IOBUF(uart0_txd_in)
  }
